...
class ETL():
    ...
    def run_extract(self, gcs_obj, response_df, source_file_name, gcs_bucket_name, gcs_blob_name, gbq_schema):
    ...
    def run_load(
        ...
        gbq_obj.load_to_gbq_from_gcs(bq_uri, gbq_table_id, gbq_write_disposition, gbq_schema)
    ...
    def run_strava_ingest(self, response_json, endpoint, **kwargs):
        ...
        table_id = self.gbq_obj.create_table_id(self.gcp_project, self.gbq_schema_name, endpoint)

        gbq_args = {
            'schema': self.settings['tables'][endpoint]['columns'],
            'table_id': table_id,
            'write_disposition': self.settings[self.gcp_env]['gbq']['tables'][endpoint]['write-disposition']
        }
        ...
        transform_args = {
            'set_values': {
                k: kwargs[k] for k in transform_settings['set_values']
            }
        }
